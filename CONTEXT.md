...
- **Last worked on:** Added `GET /search?city={name}` — live lookup for ANY Indian city (fixes the generalization gap), using free geocoding (Open-Meteo), live weather, the already-general XGBoost model, and CPCB's city filter. Wired into the frontend as a new "Search a city" page. Found and fixed a real production bug this way: Render Static Sites have no SPA fallback by default, so direct URLs/refreshes on nested routes (`/search`, `/region/:id`) returned "Not Found" — fixed with a dashboard-configured Rewrite rule (`/*` → `/index.html`), verified with direct navigation + hard refresh in production.
